...
@section('content-header')
    <h1>
        {{ trans('icommerce::taxclasses.title.edit taxclass') }}
    </h1>
    <ol class="breadcrumb">
        <li><a href="{{ route('dashboard.index') }}"><i class="fa fa-dashboard"></i> {{ trans('core::core.breadcrumb.home') }}</a></li>
        <li><a href="{{ route('admin.icommerce.taxclass.index') }}">{{ trans('icommerce::taxclasses.title.taxclasses') }}</a></li>
        <li class="active">{{ trans('icommerce::taxclasses.title.edit taxclass') }}</li>
    </ol>
@stop

@section('content')
    <!-- {!! Form::open(['route' => ['admin.icommerce.taxclass.update', $taxclass->id], 'method' => 'put']) !!} -->
    <div class="row">
        <div class="col-md-12">
                ...
                <div class="tab-content">
                    ...
                    @foreach (LaravelLocalization::getSupportedLocales() as $locale => $language)
                        ...
                        <div class="tab-pane {{ locale() == $locale ? 'active' : '' }}" id="tab_{{ $i }}">
                            @include('icommerce::admin.taxclasses.partials.edit-fields', ['lang' => $locale])
                        </div>
                    @endforeach

                    <div class="box-footer">
                        <button type="button" onclick="storeTaxClass()" class="btn btn-primary btn-flat">{{ trans('core::core.button.update') }}</button>
...
function storeTaxClass(){
  ...
  if(name==""){
    ...
    $.ajax({
      url:"{{url('/backend/icommerce/taxclasses/')}}"+"/{{$taxclass->id}}",
      type:'PUT',
