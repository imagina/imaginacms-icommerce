        ...
        <table class="data-table table table-bordered table-hover">
            ...
            <tbody class="items">
                @if(isset($optionValues))
                    ...
                    <tr>
                        <td hidden="true">
                            {!! Form::normalInput('id',trans('icommerce::options.table.id'), $errors,$value,$opInputId) !!} 
                        </td>

                        <td>
                            {!! Form::normalInput('description',trans('icommerce::options.table.description'), $errors,$value,$opInputDescription,$op) !!}  
                        </td>
                            ...
                            <div data-preview="#mainimage" data-aspectRatio="0" data-crop="0" class="form-group col-md-12 image">
                                ...
                                <div class="row">
                                    <div class="col-sm-6" style="margin-bottom: 20px;">
                                        @if(isset($value->image))
                                            <img id="mainImage" src="{{ url($value->image) }}">
                                        @else
                                ...
                                <div class="btn-group">
                                    ...
                                    <label class="btn btn-primary btn-file" onclick="image_caller()">
                                        ...
                                        @if(isset($value->image))
                                            <input type="hidden" id="hiddenImage" name="mainimage[]" value="{{ $value->image }}">
                                        @else
                                            <input type="hidden" id="hiddenImage" name="mainimage[]" value="">
                                        @endif
                        ...
                        <td>
                            {!! Form::normalInputOfType('number','sort_order',trans('icommerce::options.form.sort_order'),$errors,$value,$opInputSortOrder) !!}
                        </td>

                        <td>
                            @if(!$value->productoptionvalue)
...
@push('js-stack')
	...
	<script type="text/javascript">
		$(document).ready(function () {
			...
			if ($("#selType option:selected").val() == 'select' || $("#selType option:selected").val() == 'radio' || $("#selType option:selected").val() == 'checkbox') {
    ...
    <script>     
        $(document).ready(function () {
            ...
            var fieldHTML = 
                ...
                +
                            '<input type="text" class="form-control" value="" name="description_value[]" id="description" placeholder="descripción">' 
                + 
                        '</div>'
                +
                    '</td>'
                ...
                +
                    '<td>'
                +
                        '<input type="number" class="form-control" value="" name="sort_order_value[]" id="sort_order" placeholder="Valor de ordenamiento">'
                +
                    '</td>'
                +
                    '<td>'
            ...
            $(wrapper).on("click",".remove_field", function(e){
                ...
                var id_value = $(this).parents('tr').find('#id').val();
                console.log(id_value);

                if ( $.isNumeric(id_value) ) {
    
                    url = route('admin.icommerce.option.deleteOptionValue', {id: id_value});
                    ...
                    .error( function(data) {
                        console.log(data);
                        alert("The option value could not be deleted");
                    }); 
                }
