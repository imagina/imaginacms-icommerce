      ...
      <table role="presentation" cellpadding="0" cellspacing="0" border="0" style="width: 100%;">
        ...
        @if(!empty($order->shipping_amount))
          <tr class="shippingTotal">
            ...
            <td colspan="2" style="text-align: right;font-weight:400;color:#212529;">
              {{$order->shipping_method}} {{ $order->shipping_amount>0 ? ' - '.number_format($order->shipping_amount,2) : ''}}{{$order->currency->symbol_right ?? ''}}
            </td>
          </tr>
        @endif
        @if(!empty($order->tax_amount) && $order->tax_amount!=0)
          <tr>
            ...
            <td colspan="2"
                style="text-align: right;font-weight:400;color:#212529;">
              {{$order->currency->symbol_left ?? ''}}{{number_format($order->tax_amount,2)}}{{$order->currency->symbol_right ?? ''}}
            </td>
          </tr>
        @endif
        <tr>
  ...
  <tr>
    <td>
      <table role="presentation" cellpadding="0" cellspacing="0" border="0" style="width: 100%;">
        <tr>
          @php
            $orderTransformed = collect( new \Modules\Icommerce\Transformers\OrderTransformer( \Modules\Icommerce\Entities\Order::with( [ 'customer', 'addedBy', 'orderItems', 'orderHistory', 'transactions', 'paymentCountry', 'shippingCountry', 'shippingDepartment', 'paymentDepartment' ] )->where( "id", $order->id )->first() ) )->toArray();
