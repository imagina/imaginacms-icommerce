				  	...
				  	<ul>
				  		@foreach($option->optionValues as $key =>$value)
				  			...
				  			<div class="form-check">
						  		<input class="form-check-input" type="checkbox" value="{{$value->id}}" name="optionValues{{$value->id}}" id="optionValues-{{$value->id}}" wire:model="selectedOptionValues.{{$value->id}}" >
