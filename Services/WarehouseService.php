...
class WarehouseService
{
  ...
  public function getWarehouseToAddress($address)
  {
    ...
    //Get Global Warehouse
    $globalWarehouseId = setting('icommerce::globalWarehouse', null, null);
    ...
    foreach ($warehouses as $key => $warehouse) {
      ...
      if (!is_null($polygon)) {
        ...
        if ($this->isInPolygon($pointsPolygon, $vsX, $vsY, $address->lng, $address->lat)) {
          ...
          //Solamente si no existe un warehouse global
    ...
    //No se encontró un warehouse que cubra esa direccion, ademas tampoco uno cercano (Porque ese cercano no tiene poligono)
    if (is_null($nearbyWarehouse)) {

      \Log::info($this->log . 'getWarehouseToAddress|Asigna Global Warehouse');
      $finalWarehouse = $warehouses->firstWhere('id', $globalWarehouseId);
   ...
   */
  public function isInPolygon($numVertices, $vsX, $vsY, $lngX, $latY)
  {
    $c = false;
    for ($i = 0, $j = $numVertices - 1; $i < $numVertices; $j = $i++) {
      if (
        (($vsY[$i] > $latY) != ($vsY[$j] > $latY)) &&
