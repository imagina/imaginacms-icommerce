...
class ProductController extends AdminBaseController
{
    ...
    public function searchProducts(Request $request)
    {
        ...
        $query = Product::select('icommerce__products.id', 'icommerce__products.title', 'icommerce__products.sku', 'icommerce__products.price','icommerce__products.status', 'icommerce__products.created_at', 'icommerce__products.stock_status', 'icommerce__manufacturers.name')
            ->leftJoin('icommerce__manufacturers', 'icommerce__products.manufacter_id', '=', 'icommerce__manufacturers.id')
            ->where("parent_id", 0);
    ...
    {
        Excel::Load($request->importfile, function ($reader) {

            $excel = $reader->all();
            dd($excel);
            foreach ($excel as $i => $page) {


                switch ($page->getTitle()) {
                    case 'product':
                    case 'Productos':
                        $page->each(function ($row) {
                            $prod = $this->product->find($row->id);
                            if (count($prod)) {
                                try {
                                    $this->update($prod, $row);
                                } catch (\Exception $e) {
                                    $msg = trans('icommercce::product.bulkload.invalid field or invalid page');
                                }
                            } else {
                                try {
                                    $this->create($row);
