...
Route::prefix('/order-statuses')->group(function (Router $router) {
    $locale = \LaravelLocalization::setLocale() ?: \App::getLocale();

    $router->post('/', [
        'as' => $locale.'api.icommerce.order-statuses.create',
        'uses' => 'OrderStatusApiController@create',
        'middleware' => ['auth:api', 'auth-can:icommerce.orderstatus.create'],
    ]);
    $router->get('/', [
    ...
    $router->put('/{criteria}', [
        ...
        'middleware' => ['auth:api', 'auth-can:icommerce.orderstatus.edit'],
    ]);
    $router->delete('/{criteria}', [
        'as' => $locale.'api.icommerce.order-statuses.delete',
        'uses' => 'OrderStatusApiController@delete',
        'middleware' => ['auth:api', 'auth-can:icommerce.orderstatus.destroy'],
    ]);
    $router->get('/{criteria}', [
