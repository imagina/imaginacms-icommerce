            ...
            if (isset($filter->category) && ! empty($filter->category)) {
                ...
                if ($categories->isNotEmpty()) {
                    $query->where(function ($query) use ($categories) {
                        $query->whereRaw('icommerce__products.id IN (SELECT product_id from icommerce__product_category where category_id IN ('.(implode(',', $categories->pluck('id')->toArray())).'))')
            ...
            if (isset($filter->related) && ! empty($filter->related)) {
                ...
                if (isset($filter->categories) && ! empty($filter->categories)) {
                    ...
                    $query->orWhere(function ($query) use ($filter) {
                        $query->whereRaw('icommerce__products.id IN (SELECT product_id from icommerce__product_category where category_id IN ('.(implode(',', $filter->categories)).'))')
                    ...
                    $query->orderByRaw('FIELD(id,'.implode($filter->related).') DESC, id DESC');
